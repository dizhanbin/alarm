...
/**
 * Created by dizhanbin on 16/12/12.
 */
...
public class VideoResolutionChanger {
    ...
    public static void testCompress(){
        ...
        try {
            ...
            if( !file.exists() ) {
                NLog.i("compress source file not exist");
                return;
            }
            long size = file.length();
            String path =  new  VideoResolutionChanger().changeResolution(new File("/sdcard/a.mp4"));

            File cfile = new File(path);
            long csize = cfile.exists() ? cfile.length() : 0;
            NLog.i("compress video %d: -> %d %s",size,csize,path);
        } catch (Throwable throwable) {
            //throwable.printStackTrace();
            NLog.i("compress error:%s",throwable);
            throwable.printStackTrace();
        }
